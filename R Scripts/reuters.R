...
# Load data
data <- dataset_reuters(num_words = max_features)
...
y_train <- to_categorical(data$train$y, num_classes = 46)
y_test <- to_categorical(data$test$y, num_classes = 46)
...
epoch_size = 200 ; TT0 = dim(x)[1]; N <- floor(dim(x)[1]/epoch_size)
alpha = NC/dim(x)[1]
...
write_csv(RESULTS, "~/Desktop/safe_predict_cfar/reuters_results_05.csv")
